...
class ACMOJClient:
    ...
    def _make_request(self, method: str, endpoint: str, data: Dict[str, Any] = None,
        ...
        try:
            if method.upper() == "GET":
                response = requests.get(url, headers=self.headers, params=params, timeout=15, proxies={"https": None, "http": None})
            elif method.upper() == "POST":
                response = requests.post(url, headers=self.headers, data=data, timeout=15, proxies={"https": None, "http": None})
            else:
    ...
    def _save_submission_id(self, submission_id):
        try:
            ...
            print(f"Saved submission ID {submission_id} to {self.submission_log_file}")
